...
app.get('/api/reportes/pdf', async (req, res) => {
  ...
  try {
    ...
    if (formatoVista === 'bySemana') {
      ...
      console.log(`Dividiendo ${semanasArray.length} semanas en ${semanaGroups.length} páginas (máx ${MAX_SEMANAS_PER_PAGE} por página)`);
      ...
      semanaGroups.forEach((currentSemanas, pageIndex) => {
        ...
        UNIVERSITIES.forEach((uni, index) => {
          ...
          if (isLastPage) {
            ...
            doc.fillColor('#000000').fontSize(8).text(uniTotal.toString(), xPos + 5, yPos + 8, { 
        ...
        if (isLastPage) {
          ...
          doc.fillColor('#000000').fontSize(9).text(grandTotal.toString(), xPos + 5, yPos + 12, { 
            width: totalColWidth - 10, 
            align: 'center' 
          });
        }
