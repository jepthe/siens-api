...
const PRODUCTION_URL =
  process.env.API_URL || "https://siens-api-production.up.railway.app"; //por si no se define la variable de entorno, resuelve imagen perfil
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (usersByEmail.length === 0) {
      console.log(
        "Usuario no encontrado por correo, buscando por nombre de usuario"
      );
    ...
    if (user.cImagen) {
      ...
      const baseUrl =
        process.env.API_URL || "https://siens-api-production.up.railway.app";

      // Properly format the image URL
      if (
        user.cImagen.startsWith("http://") ||
        user.cImagen.startsWith("https://")
      ) {
        // Force HTTPS for security
        safeResponse.user.cImagen = user.cImagen.replace(
          /^http:\/\//i,
          "https://"
        );
